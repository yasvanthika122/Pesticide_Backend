...
//product quantity
const getTotalProductQuantityByCustomer = async (req, res) => {
        ...
        try {
            ...
            const customerCartQuantities = []; // Array to store total cart quantities for each customer
    
            customersWithSellerProduct.forEach(customer => {
                let totalQuantity = 0; // Initialize total quantity for each customer
                customer.cartDetails.forEach(cartItem => {
                    if (cartItem.seller.toString() === sellerId) {
                        totalQuantity += cartItem.quantity; // Add quantity of each product in the cart
                    }
                });
                // Push customer ID and total quantity to the array
                customerCartQuantities.push({
                    customerId: customer._id,
                    totalQuantity: totalQuantity
                });
            });
    
            res.send(customerCartQuantities); // Send the array with total cart quantities for each customer
        } catch (error) {
            res.status(500).json(error);
        }
    };
...
getTotalProductQuantityByCustomer,
//new.
};